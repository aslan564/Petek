...
class TargetPolicyTest {
    private val guarded = TargetPolicy(productionHosts = setOf("portal.example", " WWW.portal.example "), allowProduction = false)

    @Test
    fun `a production host is refused and the refusal names both env variables`() {
        val verdict = guarded.verify(URI("https://portal.example/login")).shouldBeInstanceOf<TargetVerdict.Refused>()

        verdict.reason shouldContain "'portal.example'"
    ...
    fun `production hosts are compared trimmed and without regard to case`() {
        guarded.verify(URI("https://www.Portal.example")).shouldBeInstanceOf<TargetVerdict.Refused>()
    }

    @Test
    fun `a production host is allowed when production is allowed explicitly`() {
        guarded.copy(allowProduction = true).verify(URI("https://portal.example")) shouldBe TargetVerdict.Allowed
    }

    @Test
    fun `other hosts, including subdomains of a production host, are allowed`() {
        guarded.verify(URI("https://staging.portal.example")) shouldBe TargetVerdict.Allowed
