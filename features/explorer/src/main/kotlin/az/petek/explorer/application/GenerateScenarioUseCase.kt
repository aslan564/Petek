...
class GenerateScenarioUseCase(
    ...
    fun compose(
        ...
        val draft =
            Campaign(
                ...
                target = targetProfile(model, settings, composer.idSources.toMap()),
                setup = setup,
    ...
    /**
     * The draft's target profile: its id sources, and for a site without companies the gate the explorer learned
     * (Faza 17-18): the sign-up and sign-in pages and their fields mapped onto the profile's keys, so testers pass the
     * gate by code with the default `sign_up` and `login` flows.
     */
    private fun targetProfile(
        model: SiteModel,
        settings: ScenarioSettings,
        idSources: Map<String, IdSource>,
    ): TargetProfile {
        if (settings.tenant != Tenant.NONE) return TargetProfile(emptyMap(), emptyMap(), idSources)
        val gate = GateMaps.of(model)
        return TargetProfile(gate.profilePaths, gate.profileSelectors, idSources)
    }
    ...
    private fun header(
        ...
        buildList {
            ...
            val kind = SiteKinds.of(model)
            add("site kind: ${kind.kind.name.lowercase()} (${kind.reason})")
            val gate = GateMaps.of(model)
            add(
                "gate: sign-up ${gate.register?.path ?: "none"}, sign-in ${gate.login?.path ?: "none"}, " +
                    "visitors ${if (gate.guest) "see pages" else "see nothing"}, verification ${gate.otp.name.lowercase()}, " +
                    "password reset ${if (gate.forgotPassword) "yes" else "no"}",
            )
            gate.blockers.forEach { add("gate blocker: ${it.name.lowercase()}") }
            gate.register
                ?.unmapped
