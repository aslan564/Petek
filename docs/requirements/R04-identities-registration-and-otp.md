...
**Status:** Implemented (Mailpit and test-API mail; IMAP and manual OTP planned) · **Plan:** Faza 1, 4, 8, 10 · **ADRs:** 0004, 0010
...
- `mail`: `MailpitMailboxTest`, `TestApiMailboxTest` (Ktor fake servers), extractor tests.
...
## Open items (Faza 10, ADR-0010)

- `ImapMailbox` for sites whose SMTP the owner does not control (catch-all or plus addressing).
- `ManualCodeMailbox`: the panel asks the owner for a code (explorer sessions only).
- Owner-provided accounts as a sign-in strategy and reuse of saved `storage_state`.
